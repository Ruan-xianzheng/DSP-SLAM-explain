...
ObjectDrawer::ObjectDrawer(Map *pMap, MapDrawer *pMapDrawer, const string &strSettingPath) : mpMap(pMap), mpMapDrawer(pMapDrawer)
{
    ...
    /*初始化两个4x4的变换矩阵SE3Tcw和SE3TcwFollow，并将它们都设置为单位矩阵。
    这两个变换矩阵通常用于表示相机坐标系（Camera Coordinate System，简称CCS）到世界坐标系（World Coordinate System，简称WCS）的变换矩阵，
    其中SE3Tcw表示当前帧的CCS到WCS的变换矩阵，
    SE3TcwFollow表示跟踪中的参考帧CCS到WCS的变换矩阵。*/
