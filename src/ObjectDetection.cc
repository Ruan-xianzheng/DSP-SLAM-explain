...
{
    Sim3Tco = Eigen::Matrix4f::Identity(); //将类中的 Sim3Tco 成员变量初始化为单位矩阵。
    Rco = Sim3Tco.topLeftCorner<3, 3>(); //将 Sim3Tco 矩阵的左上角的 3x3 子矩阵赋值给 Rco，即将 Sim3Tco 矩阵的旋转部分提取出来。
    scale = 1.; //将 scale 变量初始化为 1.0
    Rco /= scale; //将 Rco 矩阵的每个元素除以 scale
    tco = Sim3Tco.topRightCorner<3, 1>();//Sim3Tco 矩阵的右上角的 3x1 子矩阵赋值给 tco，即将 Sim3Tco 矩阵的平移部分提取出来
    // Transformation Matrix in SE3
    SE3Tco = Eigen::Matrix4f::Identity();//同理
    SE3Tco.topLeftCorner<3, 3>() = Rco;
    SE3Tco.topRightCorner<3, 1>() = tco;
