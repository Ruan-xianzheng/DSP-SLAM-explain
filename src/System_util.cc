...
void System::SaveEntireMap(const string &dir) {
    string fname_pts = dir + "/MapPoints.txt"; //将保存的地图点文件名设置为"/MapPoints.txt"，并将保存的路径赋值给变量fname_pts
    ofstream f_pts;
    f_pts.open(fname_pts.c_str());//将存放的路径转化成c++格式，并赋值给f_pts变量
    f_pts << fixed;//std::fixed是一个输出流格式标志符（std::ios_base::fmtflags），用于控制浮点数输出的小数位数和精度

    const vector<MapPoint *> &vpMPs = mpMap->GetAllMapPoints();//得到所有的地图点
    for (size_t i = 0, iend = vpMPs.size(); i < iend; i++) {
        //判断地图点是不是坏点，如果是则丢弃并继续判断下一个地图点，如何是满足要求的点，则将其标在地图对应的地方
        if (vpMPs[i]->isBad())
            continue;
        cv::Mat pos = vpMPs[i]->GetWorldPos();
        f_pts << setprecision(9) << pos.at<float>(0) << " " << pos.at<float>(1) << " " << pos.at<float>(2) << endl;
    ...
    string fname_objects = dir + "/MapObjects.txt"; //设置需要保存的地图目标物体的地址
    ofstream f_obj;
    f_obj.open(fname_objects.c_str());
    f_obj << fixed;
    auto mvpMapObjects = mpMap->GetAllMapObjects();
